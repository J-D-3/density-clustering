...
- `HnswBackend<T,Dim>` (`hnsw_backend.hpp`, behind `OPTICS_ENABLE_HNSW`; vendored hnswlib in `include/optics/hnswlib/`, #47): **approximate** HNSW graph index. Models `NeighborSearch` (radius via hnswlib's **native epsilon range search** — a single graph traversal, `EpsilonSearchStopCondition`) and `KnnCoreDist`. Stores coordinates as float; recall is tunable via `M` / `ef_construction` (defaults 16 / 200). **Measured finding (`optics_hnsw_probe`, adaptive ~30-NN radius, n≈19k): HNSW query time is essentially dimension-independent (~55–80 ms across 16–128-D) while the exact KD-tree grows with D (34 → 82 → 224 ms), so they cross over at ~64-D — HNSW is ~2.8× faster by 128-D (recall ~0.92–0.99).** At the 16-D target the exact KD-tree is still ~2× faster, so nanoflann remains the default; HNSW is the right backend for the genuinely high-D (≥64-D) regime (where the static-`Dim` KD-tree even overflows the stack to build at 256-D). A one-time index build (~2 s at this n) is HNSW's fixed cost.
...
Two **optional capabilities** are detected with `if constexpr` (a backend opts in by providing the method): `KnnCoreDist` (`knn_core_dist`, enables `CoreDistMode::Knn`) and `RadiusSearchWithDists` (`radius_search_with_dists`, #55) — returns each neighbor's squared distance alongside its index so the ordering can reuse them in the core-dist scan and the relaxation instead of recomputing. `RadiusSearchWithDists` is gated to `double` (where the backend's squared distance is bit-identical to `detail::square_dist`, so orderings stay byte-identical); `NanoflannBackend` models both.
...
`compute_soptics_reachability_dists(points, min_pts, epsilon = -1, n_projections = 1024, k = 0, m = 0, seed = 42, n_threads = 0)` is a scalable, **approximate** OPTICS (sDBSCAN/sOPTICS, Xu & Pham 2024; issue #50, reimplemented from the paper in `documentation/`). It deliberately does **not** use the `NeighborSearch` backend seam: instead of a radius search it builds **CEOs random-projection neighborhoods** (`detail/random_projection.hpp` — project onto `n_projections` Gaussian vectors, take each point's top-`k` extreme vectors, gather the top-`m` extreme points of each, ε-filter, symmetrize) and feeds them — with core-distance = the `min_pts`-th nearest candidate — into the **shared ordering driver `detail::optics_order`**. (`ceos_neighbors` also optionally returns each candidate's squared distance via `out_sq`; sOPTICS reuses those in the core-dist scan + relaxation instead of recomputing — the #55 idea on the RP path, byte-identical for float *and* double since every distance comes from `detail::square_dist`.) That driver is the algorithm-agnostic loop (seed queue, relaxation, lazy deletion) factored out of `compute_reachability_dists`; both OPTICS and sOPTICS drive it through a neighbor-provider + core-dist-provider pair (template params, so the `KnnCoreDist` `if constexpr` stays zero-overhead and orderings remain byte-identical for OPTICS). The default metric is **cosine** (points are L2-normalized onto the unit sphere, where Euclidean distance is monotone in cosine distance). A `metric` argument (`Metric::L2` / `Metric::L1`, #51) instead embeds the points into **random Fourier features** (`detail/random_features.hpp`) whose cosine geometry approximates the Gaussian / Laplacian kernel, then runs the same cosine pipeline on the features — so the ordering tracks **Euclidean / Manhattan** distance on the *original* data (`kernel_scale <= 0` ⇒ a median-distance auto bandwidth; χ²/JS not yet, see #51). Output is randomized but **deterministic in `seed`** ⇒ validated by Rand/NMI agreement with exact OPTICS, not bit-identical orderings.
...
- **Dense neighborhoods (e.g. flat-color images) are the perf trap.** With auto-`epsilon`, the average neighborhood grows ~linearly with n, so the ordering is ~**O(n²)** in time *and* (under Precompute) memory — at ~100k px the Precompute buffer is ~19 GB. The cost is in *processing* neighborhoods (`compute_core_dist` scan + relax, both O(|nbrs|)), **not** the search, so a faster/approximate backend can't fix it; the auto-eps now defaults to the knee estimator (smaller than the old uniform one, which helps), but on genuinely dense clouds also use `OnDemand` mode (the default), `CoreDistMode::Knn`, an even smaller explicit `epsilon`, or downsample. (`compute_core_dist` no longer copies the index list — it fills a reused `thread_local` of squared distances.) **#55** further trims this: on the `double` nanoflann path the ordering now *reuses* the squared distances the search already computed (the `RadiusSearchWithDists` capability) instead of recomputing them in the core-dist scan and the relaxation — ~30%/~23% off those two phases, ~12% off the loop on a dense 3-D cloud (`optics_reuse_probe`), orderings byte-identical. The search itself is still the floor.
