...
- `HnswBackend<T,Dim>` (`hnsw_backend.hpp`, behind `OPTICS_ENABLE_HNSW`; vendored hnswlib in `include/optics/hnswlib/`, #47): **approximate** HNSW graph index. Models `NeighborSearch` (radius via grow-k k-NN + filter) and `KnnCoreDist`. Stores coordinates as float. **Measured finding (`optics_hnsw_probe`): at the library's target dimensionalities (≤32-D, up to ~50k points) nanoflann's exact KD-tree is *faster* than HNSW for radius search despite HNSW's ~1.0 recall** — the KD-tree doesn't meaningfully degrade until much higher D, and the radius access pattern fights HNSW's k-NN-native design (which suits `knn_core_dist` far better than `radius_search`). So HNSW is opt-in and currently most relevant as a very-high-D escape hatch, not a default for the 16-D regime.
